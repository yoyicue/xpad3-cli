...
- `/data/local/tmp/xpad-install` v0.2.2 锁定构建；
...
`xpad-installer` 的 31317 最后回退还会逐阶段核对 boot ID、Zygote、system_server 与
SystemUI PID；任一变化立即触发本 boot 熔断并返回 75，普通重启前不再尝试。
...
xpad2 update --version 0.4.1     # 选择精确发布版本
...
boot logcat、DropBox/进程退出信息、KSU/SUU、PackageManager、清理结果和 lock 身份。
