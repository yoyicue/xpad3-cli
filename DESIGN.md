...
`--allow-shell` 是 PD3S 这条现代内核移植的明确约束。CLI 不仅检查 `/sys/module/kernelsu`，还用锁定的 `ksud debug info` 核验版本、UAPI、flags/features、LKM 与 late-load 状态。完整身份通过后，再等待最多 15 秒，以实际 `ksud module list` 成功作为控制面就绪栅栏；已经 Active 的 runtime 重跑时也不能绕过该检查。KSU 驻留后，后续特权命令切换到 `/system/bin/su`，IonStack 临时 su 只保留到事务收尾。已有未知 KernelSU-family 模块时，不进行在线切换。
...
v0.1.10 同步 late-load 控制面时序修复：runtime 身份通过后仍需实际执行 `ksud module
list`，以 250ms 间隔等待最多 15 秒；KSU 驻留后改用其 `/system/bin/su` transport，避免
继续依赖临时 shell-domain su。PD3S profile、IonStack、KSU 模块和 Manager 制品均不变；
LS12 专属 NeoZygisk、Vector 与 AICore/LUI Hook 不进入 XPad3 支持范围。
