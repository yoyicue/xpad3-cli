...
状态：v0.4.1 已实现；v0.1.1 完成 BoomInstaller 依赖身份和公开分发材料，v0.1.2
...
- `doctor`、`verify`、`cleanup`、`znxrun status` 在 native dispatch 中提前返回；前三个
  状态/验证路径以及 xpad2 对 ELF 的 `self-test` 都不能意外落入 31317。
...
```json
{
  ...
  "version": "0.2.2",
  "kind": "cli",
  "sha256": "9f1ff6b7635548a11c57b2b8a31b0b98b941773bc6e0f2f00a5c3dc98e3a5fc0",
  "size": 88152,
...
| `xpad-install` | v0.2.2 | `9f1ff6b7635548a11c57b2b8a31b0b98b941773bc6e0f2f00a5c3dc98e3a5fc0` |
...
25. 所有 xpad-install 输出运行中逐行写入事务日志，退出码 75 无条件映射为普通重启；导出包包含 incident JSONL。
