...
状态：v0.4.5 已实现；v0.1.1 完成 BoomInstaller 依赖身份和公开分发材料，v0.1.2
...
SHA-256 和 HTTPS URL。独立 `catalog.sig` 直接验证候选内嵌 catalog。HTTPS 证书验证与
Release RSA 签名缺一不可；远端 JSON 不能添加安装脚本或改变 xpad2 内置 provider。
网络暂态失败最多三次有界重试，大文件每 15 秒报告下载进度并通过 Range 从 partial 续传；
永久 4xx 立即失败，403/429 遵循服务端限流时间。
...
-> 下载 ELF 与小型 catalog.sig，或读取离线 bundle
...
离线 Release 同时提供 `xpad2-update-vX.Y.Z.zip`，包含签名 manifest、两份签名、目标
ELF 和为旧 updater 保留的匹配 cache ZIP。v0.4.5+ 优先从 ELF 自导出，不再解压重复 cache。
...
完全一致，不能成为第二条版本线。固定名 update manifest 供 Latest URL 使用。
